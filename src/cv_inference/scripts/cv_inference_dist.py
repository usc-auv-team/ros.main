...
Currently only outputs the bounding box pixel values for the first object seen 
(xmin, xmax, ymin, ymax) and distance using json format
TODO:
...
import timeit
import cv_distance as dt
import cv_angle as ag
...
def inference_and_talker():
    ...
    if(len(sys.argv) == 1):
        buoyFocalLength = dt.get_calibration_values()

    #--- END Distance Setup ---

    # ---Inference setup ---
    #need to limit GPU memory usage
                    ...
                    for index, value in enumerate(classes[0]):
                      ...
                      if scores[0, index] > 0.5:
                        ...
                        if(len(sys.argv) == 1):
                            ...
                            distance = dt.distance_to_camera(KNOWN_WIDTH, buoyFocalLength, perWidth)))
                        else:
                            # call calibration function
                            print("calibrating")
                            dt.calibrate(perWidth)
                        object_dict[xmin,ymin,xmax,ymax,(category_index.get(value)).get('name').encode('utf8')] = \
                                            scores[0, index]
                        sent_str = "{" + "\"xmin\":" + str(xmin) + ", \"xmax\":" + str(xmax) + ", \"distance\":" + str(distance) "}"
